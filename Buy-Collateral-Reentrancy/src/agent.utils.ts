...
export const reentracyLevel = (
  ...
  for (let i: number = 0; i < thresholds.length; i++) {
    ...
    if (reentracyCount < threshold) return isDangerousAndSeverity;
    isDangerousAndSeverity = [true, severity];
  }
  return isDangerousAndSeverity;
};

export const createFinding = (addr: string, reentracyCount: number, severity: FindingSeverity): Finding => {
